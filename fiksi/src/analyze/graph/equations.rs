...
use crate::collections::{CollectionExt, FxHashMap, FxHashSet, FxIndexMap};
...
struct Matching<A, B> {
    ...
    a_to_b: FxIndexMap<A, B>,
    /// Maps vertices in set B to their matched vertices in set A.
    b_to_a: FxIndexMap<B, A>,
}

impl<U, V> Matching<U, V> {
    fn new() -> Self {
        Self {
            a_to_b: FxIndexMap::new(),
            b_to_a: FxIndexMap::new(),
        }
    }
}
...
impl ExpressionGraph {
    ...
    pub(crate) fn find_strongly_connected_expressions<'g>(
        ...
        sccs.into_iter().rev().map(move |expressions| {
            let mut scc_free_variables = FxHashSet::new();
...
mod hopcroft_karp {
    ...
    pub(crate) fn find_maximum_matching<G: BipartiteGraphAToB>(
        ...
        let mut distance = FxIndexMap::<G::VertexIdA, u32>::with_capacity(graph.len_vertices_a());
    ...
    fn bfs<G: BipartiteGraphAToB>(
        ...
        distance: &mut FxIndexMap<G::VertexIdA, u32>,
...
/// an edge between A and B is matched, it is bidirectional. If a variable is unsaturated (non of
/// its edges are matched), all its edges are bidirectional.
///
...
mod tarjan {
    ...
    pub(crate) fn tarjan_pearce<G: VertexGraph>(graph: &G) -> Vec<Vec<G::VertexId>> {
        ...
        let mut root_index = FxHashMap::with_capacity(graph.len_vertices());
    ...
    fn visit<G: VertexGraph>(
        ...
        index: &mut u32,
        c: &mut u32,
        root_index: &mut FxHashMap<G::VertexId, u32>,
...
mod tests {
    ...
    fn maximum_matching() {
        ...
        let mut used_exprs = FxIndexMap::new();
